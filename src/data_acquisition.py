...
"""
def getAPIDatas(asset, interval) -> object:
...
"""
def wrappeAPIResponse(apiResponse) -> object:
...
def getDatas(asset, interval):
    print('Quering ' + asset + '\'s on interval ' + interval + 'min from Kraken')
    datas = getAPIDatas(asset, interval)
    return wrappeAPIResponse(datas)


if __name__ == "__main__":
    df = getDatas('BTCEUR', '1')
